	...
	m_tris(new GizmoTri[a_maxTris])
{
	...
	TestShaderStatus(m_programID, GL_LINK_STATUS, success, nullptr);

	// create VBOs
	glGenBuffers(1, &m_lineVBO);
	glBindBuffer(GL_ARRAY_BUFFER, m_lineVBO);
	glBufferData(GL_ARRAY_BUFFER, m_maxLines * sizeof(GizmoLine), m_lines, GL_STATIC_DRAW);


	glGenBuffers(1, &m_triVBO);
	glBindBuffer(GL_ARRAY_BUFFER, m_triVBO);
	glBufferData(GL_ARRAY_BUFFER, m_maxTris * sizeof(GizmoTri), m_tris, GL_STATIC_DRAW);
	...
	glm::vec3** vertexData /*= nullptr*/, unsigned int* a_vertexCount /*= nullptr*/)
{
	...
	glm::vec3 vX(a_vDimensions.x, 0, 0);
	glm::vec3 vY(0, a_vDimensions.y, 0);
	glm::vec3 vZ(0, 0, a_vDimensions.z);
...
void Gizmos::addCone(const glm::vec3& a_center, float a_radius, float a_endRadius,
	float a_fHalfLength, unsigned int a_segments,
	const bool& a_filled, const glm::vec4& a_fillColour /*= glm::vec4(1.f, 1.f, 1.f, 1.f)*/,
	const glm::mat4& a_transform /* = mat4::identity */,
	glm::vec3** vertexData /*= nullptr*/, unsigned int* a_vertexCount /*= nullptr*/)
{
	...
	for (unsigned int i = 0; i < a_segments; ++i)
	{
		glm::vec3 v0top(0, 0, 0);
		glm::vec3 v1top(sinf(i * fSegmentSize) * a_radius, 0, cosf(i * fSegmentSize) * a_radius);
		glm::vec3 v2top(sinf((i + 1) * fSegmentSize) * a_radius, 0, cosf((i + 1) * fSegmentSize) * a_radius);
		glm::vec3 v0bottom(0, -a_fHalfLength, 0);
		glm::vec3 v1bottom(sinf(i * fSegmentSize) * a_endRadius, -a_fHalfLength, cosf(i * fSegmentSize) * a_endRadius);
		glm::vec3 v2bottom(sinf((i + 1) * fSegmentSize) * a_endRadius, -a_fHalfLength, cosf((i + 1) * fSegmentSize) * a_endRadius);
...
void Gizmos::addSphere(const glm::vec3& a_center, int a_rows, int a_columns, float a_radius, const glm::vec4& a_fillColour, const bool& a_filled,
...
void Gizmos::TestShaderStatus(const unsigned int& a_uiShaderId, unsigned int a_uiStatus, int& a_iSuccessFlag, const char* a_shaderType)
{
	...
	if (a_uiStatus == GL_LINK_STATUS)
	{
		...
		{
			glGetProgramiv(a_uiShaderId, GL_INFO_LOG_LENGTH, &infoLogLength);
			log = (GLchar *)malloc(infoLogLength);
			glGetProgramInfoLog(a_uiShaderId, infoLogLength, &infoLogLength, log);
			sstm << "Error: Failed to  link shader!\n" << log;

			Application_Log* applog = Application_Log::Create();
			if (applog != nullptr)
			{
				applog->addLog(LOG_LEVEL::LOG_ERROR, sstm.str().c_str());
			}
			free(log);
	...
	else
	{
		...
		if (a_iSuccessFlag == GL_FALSE)
		{
			...
			log = (GLchar *)malloc(infoLogLength);
			glGetShaderInfoLog(a_uiShaderId, infoLogLength, &infoLogLength, log);
			sstm << "Error: Failed to compile " << a_shaderType << "shader!\n" << log;
